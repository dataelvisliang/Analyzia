...
class CustomPythonAstREPLTool(PythonAstREPLTool):
    ...
    def _run(self, query: str) -> str:
        ...
        try:
            ...
            if plt.get_fignums():
                # Get all current figures and save them for later display
                CustomPythonAstREPLTool._pending_figures = [plt.figure(num) for num in plt.get_fignums()]

                # Note that visualization will be displayed
                if result and str(result) != "None" and str(result).strip():
                    result = f"{result}\n\n[Visualization created - will be displayed after analysis]"
                else:
                    result = "[Visualization created - will be displayed after analysis]"

            return result if result else ""

        except Exception as e:
            error_message = f"Error executing code: {str(e)}"
            # Don't display error here, just return it
...
This framework ensures natural, helpful data analysis responses that match the user's actual needs rather than forcing every answer into the same rigid template.
    ...
    def __init__(self, openrouter_api_key=None, model=None):
        ...
        self.model = model or "nousresearch/hermes-3-llama-3.1-405b:free"
        self.llm = None
...
1. ALWAYS use the python_repl_ast tool to analyze data and create visualizations
2. When asked to analyze or visualize data, write and execute Python code immediately
3. For questions about trends, patterns, or "over time", create a matplotlib visualization
4. DO NOT ask clarifying questions - just analyze the data directly
5. Use df['REALDATE'] for time-based analysis (convert to datetime first)
6. Keep code simple and executable - focus on answering the user's question

Example for "rating over time":
```python
import matplotlib.pyplot as plt
import pandas as pd
...
plt.tight_layout()
# DO NOT use plt.show() - the framework displays plots automatically
```

IMPORTANT: Never use plt.show() in your code - plots are displayed automatically by the framework.

After creating a visualization, explain what it shows in your Final Answer.
"""
    ...
    def setup_agent(self, file_path):
        ...
        try:
            ...
            self.python_repl_tool = CustomPythonAstREPLTool(locals=VisualizationHandler.get_execution_context(self.df))
            ...
            with reasoning_container:
                ...
                with st.spinner("🔍 Analyzing your question..."):
                    ...
                    try:
                        ...
                        status_container.warning("⚙️ Switching to direct analysis mode...")
                        raw_response = self._direct_llm_fallback(prompt)

                    # Clear status messages
                    status_container.empty()

            # Display any pending matplotlib figures that were created during execution
